...
function [q, status] = inverseKinematics(DH,p_e_g,rpy,limit)  
    %% instruction
    ...
    %% input
    %Base = [1 0 0 200; 0 1 0 500 ; 0 0 1 40 ; 0 0 0 1];
    %p_e = invtTrans(Base)*[p_e_g ; 1];
    
    P_w = [RPY(rpy) p_e_g ; 0 0 0 1]*trans('z',DH(6,2))*[0 0 0 1]';
    ...
    if b^2-4*a*c >= 0 %% solution found
     %%   
        ...
        T_all = zeros([6,size(T2_can,1)*size(T2_can,2)]);
        for i = 1:size(T2_can,1)
            for j = 1:size(T2_can,2)
                T1 = T1_can(i,j);
                T2 = T2_can(i,j);
                ...
                T_all(:,2*(i-1)+j) = [T1;T2;T3;T4;T5;T6];
            end
        end
...
function map_q = Map(q)
    for i = 1:size(q,2)
        for j = 1:size(q,1)
            x = cos(q(j,i));
            y = sin(q(j,i));
            if y < 0 
                map_q(j,i) = 2*pi - (-atan2(y,x));
            else
                map_q(j,i) = atan2(y,x);
            end 
            if abs(map_q(j,i)-2*pi) < 0.001
                map_q(j,i) = 0;
            end
        end
    end
