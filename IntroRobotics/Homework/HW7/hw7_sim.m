...
q_r = @(t)[alpha1*sin(w1*t) ; alpha2*cos(w2*t)];
qd_r = @(t)[alpha1*w1*cos(w1*t) ; -alpha2*w2*sin(w2*t)];
qdd_r = @(t)[-alpha1*w1^2*sin(w1*t) ; -alpha2*w2^2*cos(w2*t)];

u = @(t,x)inverseDynamicsController(t,x,q_r,qd_r,qdd_r);
%u = @(t,x)[0;0];
[t,x] = ode45(@(t,x)dyn_hw7(t,x,u(t,x)),[0:0.01:t_max],x_init);
